...
async def upload_document(
    ...
    if document_service is None:
        raise HTTPException(
            status_code=503,
            detail="Document service is not ready",
        )
    ...
    try:
        ...
        status_code = (
            ...
            or "exceeds" in str(error).lower()
            else 400
        )

        raise HTTPException(
            status_code=status_code,
            detail="SmartDoc could not process this PDF: " + str(error),
        ) from error
...
async def upload_documents(
    ...
    for file in files:
        ...
        if not file.filename or not file.filename.lower().endswith(".pdf"):
            failures.append(
                f"{file.filename or 'unnamed file'}: "
        ...
        try:
            ...
            # Convert UploadResponse -> DocumentSummary
            uploaded.append(
                DocumentSummary(
...
def delete_document(
    ...
    if (
        document_service is None
        or not document_service.remove(document_id)
    ):
        raise HTTPException(
            status_code=404,
...
def query_document(
    ...
    if answer_service is None:
        raise HTTPException(
            status_code=503,
            detail="Answer service is not ready",
        )
...
def create_share(
    ...
    if share_service is None:
        raise HTTPException(
            status_code=503,
            detail="Share service is not ready",
        )
