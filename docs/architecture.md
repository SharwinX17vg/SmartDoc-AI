...
- `services/orchestration.py`: converts natural requests and bounded conversation context into a
  structured task plan, including topic/entities, page scope, multipart questions, response
  requirements, follow-up reference, and retrieval query variants.
...
references, supports document-grounded learning tasks (teaching, study plans, quizzes, flashcards,
revision, and exam preparation), compresses evidence, and returns concise source metadata.
Natural-language page ranges and explicit API page bounds are enforced in the retrieval adapter.
