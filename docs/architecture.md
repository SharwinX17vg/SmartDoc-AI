...
The answer service classifies casual messages before retrieval, supports bounded follow-up context, compresses evidence, and returns concise source metadata. `create_answer_provider()` selects `LLMAnswerProvider` only when `LLM_PROVIDER` and an API key are configured; otherwise it logs and selects `ExtractiveAnswerProvider`. Provider failures are logged and fall back safely without exposing provider details.
